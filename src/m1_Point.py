...
class Point(object):
    ...
    def __init__(self, x_val, y_val):
        self.x = x_val
        self.y = y_val #Instance Variables
    ...
    def move_to(self, x, y):
        dx = (self.x-x)**2
        dy = (self.y-y)**2
        self.total_distance = self.total_distance + math.sqrt(dx+dy)
        self.x = x
        self.y = y
    ...
    def move_by(self, dx, dy):
        dex = dx** 2
        dey = dy** 2
        self.total_distance = self.total_distance + math.sqrt(dex + dey)
        self.x = dx + self.x
        self.y = dy + self.y
        self.number_of_moves = self.number_of_moves + 1
    ...
    def get_distance_from_start(self):
        dx = (self.x-self.intitalx)**2
        dy = (self.y-self.initialy)**2
        self.distance = (math.sqrt(dx+dy))
        return self.distance
    ...
    def closer_to(self,P2,P3):
        self.distance1 = self.get_distance_from(P2)
        self.distance2 = self.get_distance_from(P3)
